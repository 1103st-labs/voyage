...
class make_manifest(base.base):
    ...
    async def setup(self, m):
        ...
        # set time to at 8 am
        tmp = list(time.localtime(self.last_time))
        tmp[3] = 10
        tmp = time.struct_time(tuple(tmp))
    ...
    async def program(self, m):
        if (m.content.lower() == "gen"):
            ...
            self.user.data["VMAP"].update_manifest()
            tmp = time.localtime()
            ...
            for w, r in zip(waypoint_tmp, reward_tmp):
                tmp = time.localtime(w[0])
                tmp = (f'{tmp.tm_mon}.{tmp.tm_mday}',
                       f'{tmp.tm_hour}:{tmp.tm_min}')
                index.append(tuple((embed.add_field(
                    name=f'［ 🞛  {len(index)} ］［ {w[1].des} ］ ',
                    value=f'［ ⛀ {r[0]}    ⛂ {r[1]} ］'
                          f'［ ⎆ {w[1].heading.name} ］［ 🗓 {tmp[0]}    ⏱  {tmp[1]} ］',
                          inline=False), w)))
            waypoint_tmp = \
        ...
        if (m.content.lower()[0] == "u"):
            ...
            for x in tmp[1:]:
                ...
                if (self.user.data["VMAP"].manifest["Index"][x][1][1] in
                    self.user.data["VMAP"].manifest["Mandatory"]["Waypoints"].values()):
                    self.printed_manifest._fields[x]["name"]\
                        = self.printed_manifest._fields[x]["name"].replace("🞙", "🞛")
                elif (self.user.data["VMAP"].manifest["Index"][x][1][1] in
